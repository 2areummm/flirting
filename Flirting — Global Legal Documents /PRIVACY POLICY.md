2. PRIVACY POLICY


1. Introduction
This Privacy Policy explains how Flirting ("we," "us," "our") collects, uses, stores, and protects your personal data. By using Flirting, you consent to the practices described here.

2. Information We Collect
2.1 Information You Provide
Account Information: Phone number, email, name, date of birth, gender
Profile Information: Photos, bio, preferences, interests
Verification Data: Photo verification, identity documents (if applicable)
Communications: Messages, calls, and other content you share
Payment Information: Processed by Apple/Google (we don't store card details)
2.2 Information Collected Automatically
Device Information: Device model, OS, unique identifiers
Usage Data: App activity, features used, time spent
Location Data: Approximate location (with your permission)
Log Data: IP address, access times, error logs
2.3 Information from Third Parties
Social login providers (Apple ID, Google, etc.) if you choose to connect
3. How We Use Your Information
We use your data to:

Create and manage your account
Match you with other users
Communicate with you about the Service
Process payments
Improve and develop features
Detect and prevent fraud
Comply with legal obligations
Enforce our Terms and Community Guidelines
4. Legal Basis for Processing
We process your data based on:

Consent: When you actively agree (e.g., marketing, location)
Contract: To provide the Service you signed up for
Legitimate Interests: To improve safety and prevent fraud
Legal Obligation: To comply with applicable laws
5. Data Storage and International Transfer
5.1 Server Location
Your data is stored on secure servers located in the Republic of Korea.

5.2 International Data Transfer
By using Flirting, you explicitly consent to the transfer of your personal data to the Republic of Korea for processing, even if your country has different data protection laws.

We implement appropriate safeguards including:

Encryption in transit (TLS 1.2+) and at rest (AES-256)
Access controls and authentication
Regular security audits
Data minimization principles
5.3 Cross-Border Transfer Consent (Important)
For users in jurisdictions requiring explicit consent for data transfer outside their country (including but not limited to Vietnam, Indonesia, UAE, Saudi Arabia), creating an account constitutes your explicit consent to such transfer.

6. Data Sharing
We do NOT sell your personal data. We may share data with:

Service Providers: Cloud hosting, analytics, payment processing
Law Enforcement: When legally required
Business Transfers: In case of merger or acquisition
Other Users: Information you choose to display on your profile
7. Your Rights
7.1 General Rights (All Users)
You have the right to:

Access your personal data
Correct inaccurate information
Delete your account and data
Restrict processing in certain cases
Object to direct marketing
Data Portability (receive a copy in machine-readable format)
Withdraw consent at any time
7.2 Region-Specific Rights
🇯🇵 Japan (APPI): Right to request disclosure, correction, and suspension of use of your personal data. Contact privacy@flirting.com.

🇻🇳 Vietnam (PDPD 2023): Right to access, rectification, erasure, and to object to data processing. Right to be informed of data breaches affecting you.

🇲🇾 Malaysia (PDPA 2010): Right to access and correct your personal data. Right to withdraw consent. Right to prevent processing for direct marketing.

🇮🇩 Indonesia (UU PDP 2022): Right to access, rectify, erase, and restrict processing. Right to data portability. Right to withdraw consent.

🇦🇪 UAE (PDPL — Federal Decree-Law No. 45/2021): Right to access, correction, erasure, restriction, portability, and to object to processing. Right to file a complaint with the UAE Data Office.

🇸🇦 Saudi Arabia (PDPL — Royal Decree No. M/19): Right to access, correction, erasure, and to object to direct marketing. Right to withdraw consent. Right to file a complaint with SDAIA within 90 days of becoming aware of any violation.

7.3 How to Exercise Your Rights
Email privacy@flirting.com with your request. We will respond within 30 days.

8. Data Retention
We retain your data:

Active accounts: As long as your account exists
Deleted accounts: Most data deleted within 30 days
Legal/safety records: Up to 5 years as required by law
Anonymized analytics: Indefinitely
9. Security
We implement industry-standard security measures including encryption, access controls, and regular audits. However, no system is 100% secure. Report security concerns to security@flirting.com.

10. Children's Privacy
Flirting is strictly for users 18 and older. We do not knowingly collect data from anyone under 18. If we discover such data, we will delete it immediately. Parents who believe their child has provided us with personal information should contact privacy@flirting.com.

11. Cookies and Tracking
We use cookies and similar technologies for:

Authentication
Analytics
Personalization
Security
You can manage preferences through your device settings.

12. Third-Party Links
Our Service may contain links to third-party services. We are not responsible for their privacy practices.

13. Changes to This Policy
We may update this Privacy Policy. Material changes will be notified via the app or email. Continued use constitutes acceptance.

14. Contact
Privacy Officer: info@evmodu.kr General Support: info@evmodu.kr

For users in the EEA/UK, you may also lodge a complaint with your local data protection authority.
